...
```{r setup-knitr, include=FALSE, purl=FALSE}
library(knitr)
...
- [CRAN](http://cran.r-project.org/) not there yet... but soon
...
- However, **breedR** makes (intensive) use of the [`BLUPF90`](http://nce.ads.uga.edu/wiki/doku.php) suite of Fortran programs, which **are not free**
...
```{r extraction-2, warning = FALSE, fig.width = 3, fig.height = 3}
qplot(
      fitted(res),
      globulus$phe_X) +
  geom_abline(int = 0,
...
- Otherwise, in both cases we specify the same thing
```{r random-spec}
...
```{r hierarchical-exercise, purl=FALSE, warning=FALSE}
...
```{r factorial-exercise, purl=FALSE, warning=FALSE}
...
```{r interaction-exercise, purl=FALSE, warning = FALSE}
summary(res)
summary(res.h)
```
...
```{r factorial-summary, purl=FALSE, warning=FALSE}
summary(res.f)
```

```{r AIC-factorial, warning=FALSE, purl=FALSE}
...
```{r genetic_result, warning = FALSE}
summary(res.animal)
```
...
PBV <- model.matrix(res.animal)$random$genetic %*% PBV.full
...
```{r spatial-blocks, warning = FALSE}
...
```{r genetic-spatial_result, warning = FALSE}
summary(res.blk)
```
...
```{r spatial-exercise-1, purl = FALSE, warning=FALSE}
...
```{r spatial-exercise-1-results, purl = FALSE, warning = FALSE, echo = FALSE}
summary(res.spl)
summary(res.spl99)
```
...
```{r spatial-exercise-2, purl = FALSE}
qplot(rho_r, rho_c,
      fill = loglik,
      geom = 'tile',
      data = res.ar1$rho)
...
```{r spatial-exercise-3, purl = FALSE, warning=FALSE}
...
```{r competition-fit, warning = FALSE}
system.time(
...
## TODO: This plot has changed, and it should not have. Check.


## Competition #1 | True vs. predicted components
```{r competition-exercise-1, purl = FALSE, message=FALSE}

## compute the predicted effects for the observations
## by matrix multiplication of the incidence matrix and the BLUPs
pred <- list()
Zd <- model.matrix(res.comp)$random$'genetic-direct'
pred$direct <- Zd %*% ranef(res.comp)$'genetic-direct'
...
pred$comp <- Zd %*% ranef(res.comp)$'genetic-competition'
pred$pec  <- Zd %*% ranef(res.comp)$pec
...
```{r prediction-fit, echo = FALSE, warning=FALSE}
...
## by matrix multiplication of the incidence matrix and the BLUPs
Zd <- model.matrix(res.comp)$random$'genetic-direct'
pred.BV.loo.mat <- with(ranef(res.comp.loo), cbind(`genetic-direct`,
                                               `genetic-competition`))
...
```{r prediction-exercise-1, purl = FALSE, results = 'asis', echo = -4}
pred.BV.mat <- with(ranef(res.comp), cbind(`genetic-direct`,
                                           `genetic-competition`))

pred.genetic.loo <-
  Zd[rm.idx, ] %*% with(ranef(res.comp),
                        cbind(`genetic-direct`,
                              `genetic-competition`))
...
```{r prediction-exercise-2, purl = FALSE, warning = FALSE, echo = 1:4, results = 'asis'}
...
```{r prediction-exercise-3, purl = FALSE}
