...
# pheno_ped <- read.table("pheno_ped_case6.txt", header = T)
pheno_ped <- read.table("pheno_ped.txt", header = T)
...
genot <- read.table("genotypes.txt", sep = " ", header = F)
# removes self from file, follows same order as production file
genot_no_self <- genot[c(-1)]

# # get a subsample of markers
# seq_markers <- seq(n_SNP)
# samp_markers <- sample(seq_markers, 17)
# genot_no_self <- genot_no_self[,samp_markers]
...
gsmod_phe_X <- remlf90(fixed   = phe_X ~ 1,
                     generic = list(gen = list(gen.inc.alt, gen.cov, var.ini = 10)),
                     method  = 'em',
                     data    = pheno_ped4,
                     var.ini = list(resid=10))
summary(gsmod_phe_X)
...
# tiff(filename="scatterplot_TBV_vs_EBV_GS.tif",width=500,height=500,compression = c("none"))
...
# tiff(filename="scatterplot_TBV_vs_EBV_ped.tif",width=500,height=500,compression = c("none"))
...
# tiff(filename="heatmap_A.tif",width=500,height=500,compression = c("none"))
  heatmap(A[founders:num_tot,founders:num_tot])
# dev.off()
