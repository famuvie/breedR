...
renderpf90.effect_group <- function(x) {
  ...
  list(
    nlevels   = concatenate(aux, 'nlevels'),
...
#' Render generic effects into pf90 code
#' 
#' Compute the parameters of a progsf90 representation of a generic effect.
#' 
#' All matrices are converted to plain matrix-class, for exporting to files.
#' @return Number of levels and type for each 'virtual' effect; model
#'   \code{user_file} or \code{user_file_i} as appropriate; a file name and its
#'   content.
